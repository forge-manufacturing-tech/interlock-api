...
def get_agent(api_key: str | None = None):
    ...
    ## Tools
    Use the local API running at {base_url} to interact with the program
    and perform operations.{auth_doc}

    **IMPORTANT:** You can ALWAYS fetch the API schema by checking:
    `res = http_get("{base_url}/openapi.json")`
    ...
    **CRITICAL:** Do NOT just show the user a plan or a python script. Your job is to DO it.
    1. First, always fetch the openapi schema `http_get("{base_url}/openapi.json")` and print it using `run_agent_code` so you know the exact URLs and schemas.
